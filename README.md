...
- Première couche API V2 (optionnelle) : `GET /api/health/cloud`, `POST /api/checkout`, `GET /api/purchase/complete`, `POST /api/webhooks/stripe` — voir [`docs/V2_SETUP.md`](docs/V2_SETUP.md) ; checklist mémo (Stripe + reste) : [`docs/V2_CHECKLIST.md`](docs/V2_CHECKLIST.md). Les CTA **Tarifs**, **Résultats** (pack) et **Rapport** appellent le checkout quand Stripe + Price IDs sont configurés ; après paiement : upsert **`purchases`**, sync **`premium_reports`** si snapshot checkout, cookie httpOnly et persistance si Supabase est branché.
