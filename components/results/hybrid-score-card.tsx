...
export function HybridScoreCard({
  ...
  score: number;
  label: string;
  /** Si fourni et strictement inférieur à 55 %, affiche l’explication d’amortissement. */
  reliabilityPct?: number;
}) {
  const showDampenHint =
    typeof reliabilityPct === "number" && Number.isFinite(reliabilityPct) && reliabilityPct < 55;
  return (
    <div className="rounded-2xl border border-border/90 bg-gradient-to-b from-surface/95 to-surface/80 p-6 shadow-[inset_0_1px_0_rgba(255,255,255,0.05)] lg:p-8">
      ...
      <p className="mt-1 text-xs leading-relaxed text-muted/90">
        ...
        et core, pondéré selon ton objectif.
      </p>
      ...
      {showDampenHint ? (
        <p className="mt-2 text-[11px] leading-snug text-muted">
          Fiabilité encore limitée ({reliabilityPct} %) : le score affiché est légèrement amorti vers le neutre
          pour rester prudent — complète davantage de tests pour une lecture plus représentative.
        </p>
      ) : null}
      <div className="mt-6">
